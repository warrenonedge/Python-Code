...
for i in range(len(filenames)): # control range to something smaller like range(3) to avoid processing all files at one shot
    if 'new' not in filenames[i]: # all data files start with 'new'
        ...
        resultfilename = filenames[i].split('.')[0] + '_results_kmeans_random.csv'
        estimatorsfilename = filenames[i].split('.')[0] + '_estimators_kmeans_random.pickle'
        ...
        estimators = []
        for n_centers in [3,4,5,6,7,10,15]:
            estimator = KMeans(init='random', n_clusters=n_centers, n_init=10)
